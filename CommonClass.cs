...
namespace PDMS
{
    class CommonClass
    {
        ...
        {         
            ...
            string PathDeviceDirectory = Path.GetDirectoryName(path); // директория добавляемого КД
            ...
            if (true) //возможно нужно убедиться что это спецификация, а не, например ВП, прочитав шаблом документа ".spw"
            {
                ...
                if (Str == null) // Выполняем поиск данного изделия в списке объектов (в RegFile), если не нашли, то продолжаем добавление
                {
                    ...
                    DeleteSpaseToPath(ref MarkName); // удалить пробелы в конце строки

                    string RelativePath = MarkName;  //относительный путь, который будет записывать в SpcFile                

                    List<DrawObj> DrawList = regFile.transform(kompas.ReadSpc()); // Лист изделий из СП //Читаем спецификацию (Необходимо дополнить объект objspc путями к файлам)
                            ...
                            case "Документация":
                                {
                                    ...
                                    if (ex == ".spw")
                                        ...
                                        kompas.SafeFileToPDF_2D(Path.Combine(PathDeviceTemp, FileNameExt + ".pdf"));
                            ...
                            case "Детали":
                                {
                                    ...
                                    string SDO = regFile.SearchDrawObj(DrawList[i].mark); //Существует ли данное изделие в реестре

                                    if (SDO != null) //Если да то копируем ссылку
                                    {
                                        DrawList[i].path = SDO;  //Копируем ссылку на изделие
                                    }
                                    else
                                    {
                                        //Иначе пытаемся найти изделие в папках пользователя
                                        ...
                                        if (Dir == null)
                                        {
                                            MessageBox.Show("Не найден чертеж изделия " + DrawList[i].name + " " + DrawList[i].mark + ". Укажите его вручную");
                                        ...
                                        kompas.OpenFile(Dir, true);
                                        kompas.SafeFileToPDF_2D(PathDeviceTemp + "\\" + FileNameExt + ".pdf");
                                                                              
                                        DrawList[i].path = RelativePath + "\\" + FileNameExt;  //запишим путь к файлу для добавления в SpcFile //здесь возможно нужно без расширения
                                        CopyFileDraw(Dir, PathDeviceTemp + "\\" + FileNameExt); //копируем объект

                                        regFile.Drawobj = DrawList[i];
